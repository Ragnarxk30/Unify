...
struct CreateEventView: View {
    ...
    // ⬇️ lokale Gruppen-Quelle (wird mit initialen Gruppen befüllt)
    ...
    // ⬇️ Custom init, damit wir initiale Gruppen übergeben können
    init(allGroups: [AppGroup], onCreated: @escaping () -> Void) {
        self._groups = State(initialValue: allGroups)   // schon geladene Gruppen
    ...
    var body: some View {
        ...
        // ⬇️ falls Gruppen noch nicht da sind, hier nachladen
        .task {
            if groups.isEmpty {
                await loadGroups()
            }
